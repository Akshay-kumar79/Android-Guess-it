...
import com.plcoding.doodlekong.ui.setup.adapters.RoomAdapter
    ...
    private fun listenToEvent() = lifecycleScope.launchWhenStarted {
        viewModel.setupEvent.collect { event ->
            when (event) {
                ...
                is SelectRoomViewModel.SetupEvent.GetRoomErrorEvents -> {
                    binding.apply {
    ...
    private fun subscribeToObservers() = lifecycleScope.launchWhenCreated {
        viewModel.rooms.collect { event ->
            when (event) {
                ...
                is SelectRoomViewModel.SetupEvent.GetRoomEvents -> {
